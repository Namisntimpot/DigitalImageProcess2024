...
void Histogram::Equalize() {
	unsigned int* cumulation = new unsigned int[MAX_COLOR_LEVEL * channels]();
	for (int i = 0; i < channels; ++i) {
		cumulation[i] = m_histdata[i];
	}
	// 计算概率分布函数
	for (int i = 1; i < MAX_COLOR_LEVEL; ++i) {
	...
	// 计算转换表.
	m_level_map_table = new unsigned char[MAX_COLOR_LEVEL * channels];
	int hw = height * width;
	for (int i = 0; i < MAX_COLOR_LEVEL * channels; ++i) {
